...
\subsubsection{Classification}
This component is a Python batch script that archives finalized EPRs from Smartsheet to Box and prepares rows for the next review cycle.

\subsubsection{Processing Narrative (PSPEC)}
The script performs the following steps:
\begin{enumerate}[nosep]
  \item Connects to Smartsheet using an API access token loaded from the environment.
  \item Queries the EPR Tracker sheet for rows whose \texttt{Status} is \texttt{``Saving to Box''}.
  \item For each matching row, uploads the latest attached EPR PDF to Box using the Box helper module.
  \item Copies the row to a Smartsheet history sheet to preserve an immutable EPR record.
  \item Resets status, due dates, probation fields, supervisor assignments, and attachments so the row is ready for the next EPR cycle.
\end{enumerate}
...
\begin{itemize}[nosep]
  \item \textbf{Inputs:}
    \begin{itemize}[nosep]
      \item Environment variable \texttt{SMARTSHEET\_ACCESS\_TOKEN}.
      \item Smartsheet sheet IDs \texttt{SHEET\_ID} (EPR Tracker) and \texttt{HISTORY\_SHEET\_ID}.
      \item Rows with status \texttt{``Saving to Box''} and at least one EPR attachment.
    \end{itemize}
  \item \textbf{Outputs:}
    \begin{itemize}[nosep]
      \item EPR PDF files stored in Box with the naming pattern \texttt{LASTNAME-FIRSTNAME-YYYY-MM-DD.pdf}.
      \item New rows added to the EPR history sheet with a saved date stamp.
      \item Updated rows in the EPR Tracker sheet with reset fields and next EPR due date.
    \end{itemize}
  \item \textbf{External Interfaces:}
    \begin{itemize}[nosep]
      \item Smartsheet REST API (via \texttt{smartsheet-python-sdk}).
      \item Box API (via \texttt{box\_helper.upload\_file\_to\_box\_by\_url}).
    \end{itemize}
\end{itemize}
...
\begin{itemize}[nosep]
  \item \texttt{get\_smartsheet\_client()} – validates the Smartsheet access token and returns a client instance.
  \item \texttt{get\_rows\_awaiting\_saving()} – filters EPR Tracker rows with status \texttt{``Saving to Box''}.
  \item \texttt{save\_epr\_attachments\_to\_box()} – uploads the latest EPR attachment for each row to Box, tracking errors in \texttt{error\_map}.
  \item \texttt{copy\_smartsheet\_rows\_to\_history\_table()} – copies successful rows to the history sheet and stamps the saved date.
  \item \texttt{reset\_columns\_for\_next\_epr\_due\_date()} – recalculates the next EPR due date based on employment status and probation quarter, resets status and approvals, and removes attachments.
  \item \texttt{main()} – orchestrates the full workflow, enforces error handling, and logs summary results.
\end{itemize}
