...
0.1 & 10/16/2025 & Momoka Aung, Chiemela Eziechile & Initial draft. \\ \hline
...
0.3 & 12/01/2025 & Momoka Aung &  Added detailed information of System Strategy and Architecture part. \\ \hline
0.3 & 12/03/2025 & Momoka Aung &  Updated the documents with work flow diagrams and automation flow diagrrams. \\ \hline
0.3 & 12/03/2025 & Momoka Aung &  Update Box Upload function to the current version. \\ \hline
\end{tabularx}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item Includes integration details for Smartsheet, Box.com, and Power BI.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{System Integrators} – to configure Smartsheet, Box, and Power BI integrations.
\end{itemize}
...
\begin{itemize}[nosep]
    \item \textbf{Cloud Infrastructure:} All major components (Smartsheet, Box.com, Power BI) are cloud-hosted and accessible via secure web connections, no on-premises servers are required.
    \item \textbf{Existing Licenses and APIs:} The SBC PDO HR department maintains valid enterprise or educational licenses for Smartsheet, Box, and Power BI, including API access.
...
\begin{itemize}[nosep]
    \item \textbf{Platform Dependencies:} The solution relies on SaaS platforms (Smartsheet, Box, Power BI); therefore, system uptime and API availability are constrained by vendor SLAs.
    ...
    \item \textbf{Performance Requirements:} Automation latency (Smartsheet triggers → Box) should not exceed typical API round-trip times within reasonable time.
...
\begin{itemize}
  \item \textbf{3.1.1} The system is built using a combination of enterprise-grade platforms — Smartsheet, Box.com, and Microsoft Power BI.
...
\begin{itemize}
  ...
  \item \textbf{3.2.2} Smartsheet serves as the workflow backbone, Box handles storage, and Power BI provides analytics, reducing duplication and ensuring data consistency.
...
\begin{itemize}
  \item \textbf{3.3.1} Smartsheet automation drives the EPR lifecycle — identifying due reviews, routing for approval, and notifying HR staff.
...
\begin{itemize}
  \item \textbf{3.4.1} The system securely integrates Smartsheet, Box.com, and Power BI using native connectors and APIs.
  \item \textbf{3.4.2} Smartsheet end out email with attached form for signature routing, and finalized EPRs are uploaded to Box.
...
\begin{itemize}
  ...
  \item \textbf{3.6.2} If Box integration fails, the system logs errors in Smartsheet and notifies HR admins automatically.
...
\begin{itemize}
 \item \textbf{EPR Workflow Subsystem (Smartsheet)}\\
  This subsystem acts as the operational hub for tracking, automating, and managing all performance review activities. It identifies upcoming reviews, assigns reviewers, tracks status updates, and triggers automated notifications or escalations. Send out email with a link to fillable form to users to get information and signatures. This ensures that review forms are securely distributed to supervisors, HR, and leadership in sequential order, maintaining audit trails and approval records.
 ...
 \item \textbf{Integration and Data Services Layer}\\
  This layer enables secure data flow between Smartsheet, Box.com, and Power BI. It uses native connectors, REST APIs, and scheduled refreshes to ensure synchronization and data integrity.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{2)} Upon supervisor submission, Smartsheet routes the document sequentially to HR and the Head of Office for approval.
  \item \textbf{3)} Once all signatures are obtained, Smartsheet automatically uploads the finalized PDF to Box.com and updates the tracker with a Box URL and completion metadata.
  \item \textbf{4)} Power BI refreshes nightly to reflect updated completion metrics, overdue counts, and performance summaries.
  \item \textbf{5)} Automated emails notify reviewers of pending actions, overdue items, or completion confirmations.
\end{itemize}
...
Native Smartsheet, Box, and Power BI connectors were used. All integrations rely on
...
requirement is linked to specific Smartsheet columns, automation rules,
...
Individual automations and Power BI measures were tested using mock
data to ensure each component behaved correctly before connecting to live HR sheets.

\subsubsection{Integration Testing}
Full end-to-end tests were conducted across Smartsheet~$\rightarrow$~Smartsheet~$\rightarrow$~Box~$\rightarrow$~Power~BI.
...
Smartsheet use webhook-style callbacks. Audit logs follow county retention
policies.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Metadata-first design:} Identifier fields flow through each component to support
        traceability.
  \item \textbf{Separation of concerns:} Smartsheet for workflow, Box for storage, Power BI for analytics.
\end{itemize}
...
HR administrators manage Box permissions as needed.
...
The system was initially tested with mock HR data. Once validated, the Smartsheet sheets, Box folders, and Power BI reports were linked to production data. Power BI
...
\begin{itemize}[nosep]
  ...
  \item OAuth tokens for integrations (e.g., Box) must be periodically refreshed.
\end{itemize}
...
\subsubsection{Uses/Interactions}
\begin{itemize}[nosep]
  \item Receives uploads from Smartsheet after signeture completion.
...
\subsubsection{Responsibilities}
\begin{itemize}[nosep]
  \item Manages secure communication and synchronization among Smartsheet, Box, and Power BI.
...
including Smartsheet triggers, supervisor approval routing,
Python post-processing, and row reset operations.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{low_flow_chart.png}
    \caption{Detailed EPR Automation Logic (Smartsheet, Python)}
\end{figure}
...
\begin{itemize}[nosep]
  ...
  \item Triggering Smartsheet to send out email(s) to the users for signature collection.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Output Actions:} Automated email notifications, status updates, email trigger events, cell updates, and Box/Power BI synchronization.
\end{itemize}
...
They handle automation, data synchronization, analytics computation, and compliance logging—ensuring that the system remains maintainable, auditable, and efficient across Smartsheet, Box, and Power BI integrations.
...
\begin{itemize}[nosep]
  ...
  \item Power Automate handles synchronization across Smartsheet, Box, and Power BI.
\end{itemize}
...
Users interact with the system primarily through four existing platforms: \textbf{Smartsheet}, \textbf{Box}, and \textbf{Power BI}.
...
Each interface is user-friendly and designed for non-technical users. HR staff receive an email notification from Smartsheet when an employee’s EPR is due in 60 days. HR then enters the employee’s supervisor(s), assigned HR reviewer(s), and department head into the system. Supervisors receive Smartsheet email notifications prompting them to review and sign digital forms. Once completed, the forms are automatically routed to HR for verification.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Routing:} Automated Smartsheet triggers send an email attached EPR forms for HR stuff and supervisor(s).  
  \item \textbf{Completion:} Once all parties sign and approved, Smartsheet routes the finalized document to Box, where it is archived in the correct folder path.  
  \item \textbf{Analytics:} Smartsheet data refreshes nightly in Power BI, updating KPIs, completion rates, and overdue alerts.
\end{enumerate}

Each stage provides immediate visual feedback: status updates and email confirmations in Smartsheet, upload verification in Box, and live metrics in Power BI.  
