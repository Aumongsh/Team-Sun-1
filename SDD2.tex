...
This section documents concrete design decisions and day-to-day development practices that guided the implementation of the Intelligent HR Analytics \& EPR Management Dashboard. These choices do not alter the high-level architecture but directly affect interface behavior, maintainability, usability, and compliance. Each area notes rationale, alternatives considered, and justification.

\subsection{Specific Products Used}
\subsubsection{Development Environment}
Power BI Desktop/Service for analytics, and Smartsheet for workflow and data tables. Rationale: fastest delivery with enterprise-approved SaaS; alternative (custom web app) rejected due to schedule and security review overhead.
\subsubsection{Signature and Routing}
DocuSign for sequential approvals with audit trails; native Smartsheet--DocuSign connector selected over custom API glue to reduce maintenance.
\subsubsection{Storage}
Box.com for finalized documents with folder-level RBAC and retention; Smartsheet as system-of-record for tracking metadata.
\subsubsection{Connectors}
Native Smartsheet, DocuSign, Box, and Power BI connectors preferred over bespoke scripts; OAuth~2.0 for all service principals.

\subsection{Requirements Traceability}
A living traceability matrix links each SRS requirement to implementation artifacts: Smartsheet columns/sheets, automation rules, DocuSign templates, Box folder policies, and Power BI report pages. The matrix is updated each sprint and validated during sprint reviews; gaps trigger backlog items.

\subsection{Testing the Software}
\subsubsection{Unit Testing}
Rule-level tests for Smartsheet automations with mock rows; validation of DocuSign template variables; DAX measure tests in Power BI using sample datasets.
\subsubsection{Integration Testing}
End-to-end “review happy path” and “overdue escalation” scenarios across Smartsheet~$\rightarrow$~DocuSign~$\rightarrow$~Box~$\rightarrow$~Power~BI; idempotency verified by replaying events.
\subsubsection{User Acceptance Testing}
HR staff and supervisors exercise common tasks in a sandbox workspace; feedback collected via structured forms and instrumented usage logs.
\subsubsection{Accessibility Testing}
Color-contrast checks on visuals, keyboard navigation of filters, PDF exports checked with screen readers.
\subsubsection{Data Validation}
Daily checks compare Power BI aggregates with Smartsheet source counts; discrepancies \textgreater{}1\% raise alerts.

\subsection{Engineering Trade-offs}
\subsubsection{Low-Code vs.\ Custom Code}
Low-code accelerates delivery and governance alignment but constrains UI/ETL flexibility, custom services were deferred to avoid owning infrastructure.
\subsubsection{Near Real-Time vs.\ Scheduled Refresh}
Nightly Power BI refresh chosen to limit API load and licensing costs, on-demand refresh is available for critical periods.
\subsubsection{Single Source of Truth}
Smartsheet designated SoT for EPR status, Box holds immutable artifacts. Alternative “Power BI as SoT” rejected.

\subsection{Guidelines and Conventions}
\begin{itemize}[nosep]
  \item Naming: ??????????????????????
  \item Solutions: logical grouping (Workflow, Signing, Storage, Analytics).
  \item Status values normalized (\texttt{NotStarted}, \texttt{InProgress}, \texttt{PendingSignatures}, \texttt{Complete}, \texttt{Overdue}). ??????????????????????????
  \item Dates stored ISO~8601 (UTC) with display localized in reports.
  \item Box metadata tags: ????????????????????????????
  \item All changes peer-reviewed, change log kept in the traceability matrix.
\end{itemize}

\subsection{Protocols}
All inter-system calls over HTTPS; OAuth~2.0 with least-privilege scopes; webhook-style callbacks where supported; audit logs retained per county policy.

\subsection{Design Patterns and Programming Idioms}
\begin{itemize}[nosep]
  \item \textbf{Event-driven workflow}: due-date rules trigger EPR creation and routing.
  \item \textbf{Idempotent actions}: re-runs check for existing DocuSign envelopes/Box files before creating new ones.
  \item \textbf{Retry with backoff}: transient API errors retried up to three times.
  \item \textbf{Metadata-first}: identifiers flow through all steps for traceability.
  \item \textbf{Separation of concerns}: workflow (Smartsheet), signing (DocuSign), storage (Box), analytics (Power BI).
\end{itemize}

\subsection{Maintaining the Software}
Backups of Smartsheet sheets ???????? and Box retention policies ?????????, Power BI datasets monitored for refresh failures, quarterly review of automation rules with HR.

\subsection{Interfaces}
End users operate in Smartsheet (tracker views) and Power BI (dashboards). HR admins manage DocuSign templates and Box permissions; optional PowerApps surface for structured submissions.

\subsection{Source Organization}
\begin{itemize}[nosep]
  \item Smartsheet: Employees table and Employee History Records
  \item DocuSign: ?????????????
  \item Power BI: Data model organized into logical folders aligned with each dashboard page (e.g., HR Overview, EPR Performance Review), with measures and visuals grouped by purpose for clarity and easier maintenance.
  \item Box: ?????????????????
\end{itemize}

\subsection{Build and Deployment Process}
\begin{itemize}[nosep]
  \item The system was built and tested using mock HR data to verify functionality and data flow before connecting to real production sources.
  \item Smartsheet, DocuSign, Box, and Power BI components were configured and linked using test datasets to ensure all integrations worked correctly.
  \item After testing, Power BI reports were published to the HR workspace and connected to the live Smartsheet data with a scheduled daily refresh.
\end{itemize}


\subsection{Abstraction}
External services are connected through configurable connectors and rules. Power BI reports use defined measures for consistency, and PowerApps (if used) interacts with Smartsheet through a simple action layer.
These practices make the system easier to maintain, scalable for future updates, and compliant with SBC PDO IT and records policies.

\noindent\textit{Together, these practices keep the solution scalable, auditable, and maintainable while conforming to SBC PDO IT and records policies.}
