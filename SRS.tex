...
\usepackage{booktabs}
\usepackage{enumitem}
\usepackage{float}            % if you use [H] floats
...
The Software Requirements Specification for the Employee Performance Review (EPR) Dashboard Workflow System outlines the functional and nonfunctional requirements for automating and managing employee performance reviews at the Santa Barbara County Public Defender’s Office. It serves as a formal agreement between stakeholders and developers, defining the system’s objectives, scope, and operational framework.

\subsection{Purpose}
This Software Requirements Specification defines the functional and nonfunctional requirements for the Employee Performance Review (EPR) Dashboard Workflow System developed for the Santa Barbara County Public Defender’s Office.  
The purpose of this system is to automate and centralize the tracking, follow-up, and completion of employee performance reviews across all divisions. The system ensures timely completion of reviews through automated reminders, escalation notifications, and structured data management. It replaces the current manual, email-based tracking process with a transparent, repeatable, and auditable workflow using Smartsheet for process automation and Box for secure document storage.

\subsection{Intended Audience and Reading Suggestions}
\begin{itemize}
  \item \textbf{HR Staff \& Product Owner:} Configure schedules/templates; monitor compliance (Sections 1–2, 4).
  \item \textbf{Supervisors \& Division Leads:} Execute review workflow and respond to reminders (Sections 2–4).
  \item \textbf{Developers/Integrators:} Implement Smartsheet automations and email notifications; Box linkage (Sections 3–4).
  \item \textbf{QA/Reviewers \& IT Security:} Verify behavior and policy alignment (Sections 4–6).
\end{itemize}

\subsection{Product Scope}

The Employee Performance Review (EPR) Dashboard and Workflow System is developed to automate and centralize the management of employee performance reviews within the Santa Barbara County Public Defender’s Office. It replaces the current manual, email-based process with an organized and efficient workflow using Smartsheet for tracking and Box for secure document storage.

The system maintains a centralized tracker containing employee details, supervisors, review periods, due dates, and completion status. Automated reminders are sent to supervisors and employees 30, 14, and 7 days before the review due date, with follow-up escalations at 7 and 14 days after if the review remains incomplete. 

Supervisors complete EPR forms offline using HR-provided templates and upload finalized documents to Box, which serves as the official record repository. Each completed review is linked back to its Smartsheet entry for tracking and verification.

The EPR Dashboard provides HR with a clear overview of review progress, overdue items, and completion rates. By automating reminders, simplifying document management, and consolidating information in one place, the system improves efficiency, accountability, and compliance across all divisions.
...
\begin{itemize}
  ...
  \item \textbf{Smartsheet:} Workflow and tracking platform used for automations and status
  \item \textbf{Box:} County-secured document repository for finalized EPRs
  \item \textbf{SSO:} Single Sign-On for authenticated access
\end{itemize}

\subsection{References}
\begin{itemize}
  \item \href{https://cosantabarbara.app.box.com/s/xzxniv8dk6gt2eormc0b8ihsn686cf6l/file/1968556672300}{Project Overview: Box Link}
\end{itemize}
...
The section on the overall description provides a high-level understanding of the EPR Dashboard and Workflow System, including how it is intended to be used, the environment in which it will operate, and the major factors influencing its development and deployment.
...
\begin{itemize}
  \item \textbf{Smartsheet:} Serves as the central workflow hub for tracking employee data, review timelines, and completion statuses. It also manages automated reminders and overdue escalations.
  \item \textbf{Box:} Acts as the official repository for finalized EPR documents. Each document is stored in a standardized folder structure, organized by year, division, and employee name, ensuring consistent access and compliance with retention policies.
...
\begin{itemize}
  ...
  \item \textbf{2.3.4 Dashboard and Oversight}
  \begin{itemize}
    \item Provide HR with a Smartsheet dashboard showing real-time review progress.
    \item Display counts of Not Started, In Progress, Submitted, and Complete reviews.
    \item Highlight overdue items for quick follow-up and compliance reporting.
  \end{itemize}
\end{itemize}
...
2.5.1 The EPR Dashboard and Workflow System will operate entirely within a cloud-based environment using the County’s enterprise licenses for Smartsheet and Box.

2.5.2 The system will be accessible through modern web browsers, including Google Chrome, Microsoft Edge, and Mozilla Firefox.
...
2.5.5 The system will integrate with County email (Microsoft Outlook/Exchange) to send automated reminders and escalation notifications.

2.5.6 Access will be restricted to authorized employees, supervisors, HR staff, and administrators within the Santa Barbara County Public Defender’s Office.
...
The EPR Dashboard and Workflow System will include a comprehensive set of user documentation to support onboarding, daily operation, and long-term maintenance. This Software Requirements Specification (SRS) will serve as a reference guide for understanding the system’s structure, functionality, and configuration procedures.  

In addition to this document, users will receive a detailed User Guide, a Quick Start Guide, and a Frequently Asked Questions (FAQ) document to help them navigate Smartsheet dashboards, update review statuses, and upload finalized EPRs to Box. A live demonstration during the project presentation will introduce HR staff and supervisors to the system’s key features, such as automated reminders, overdue escalations, and Box link updates.  

All user materials will be distributed in PDF format and stored in a shared County Box folder to ensure consistent access and version control. Contextual help links will also be embedded directly into Smartsheet dashboards, allowing users to access relevant instructions without leaving the platform.
...
Phase 1: EPR workflow \& EPR dashboard. Phase 2: broader HR trackers, extended KPIs, drill-through.

% ===================== 3. External Interface Requirements =====================
\section{External Interface Requirements}

This section outlines how the EPR Dashboard and Workflow System will interact with external interfaces, including user interfaces, hardware, software, and communication channels. These requirements describe how the system will operate within the Santa Barbara County Public Defender’s Office’s IT environment and ensure seamless integration with existing enterprise tools.

\subsection{User Interfaces}

3.1.1 The system shall provide a clean and user-friendly interface within Smartsheet, allowing HR staff and supervisors to view, update, and manage EPR records efficiently.

3.1.2 Smartsheet forms shall enable users to input key data such as employee name, supervisor, review period, due date, and Box link for finalized performance review documents.

3.1.3 The system shall include a visual dashboard summarizing review progress and displaying key metrics such as the number of EPRs Not Started, In Progress, Submitted, Completed, or Overdue.

3.1.4 The interface shall clearly highlight overdue items using color indicators or status flags to assist HR in monitoring compliance and initiating follow-ups.

3.1.5 The Box interface shall organize completed EPR files in structured folders (/EPR/<Year>/<Division>/<Employee Name>/) and display metadata including file name, upload date, and reviewer information.
...
3.1.8 Embedded help links shall be available within Smartsheet dashboards to provide users with quick access to the User Guide, FAQs, and troubleshooting instructions.

\subsection{Hardware Interfaces}

The EPR Dashboard and Workflow System does not require any dedicated hardware and operates entirely in the cloud. Users can access the system using standard County-approved devices such as desktop computers, laptops, or mobile devices with internet connectivity. No local installation or specialized equipment is needed for normal operation, ensuring ease of access and minimal maintenance requirements.


\subsection{Software Interfaces}

The system integrates with existing County enterprise software through secure, supported connections. Smartsheet serves as the main workflow platform, while Box is used for document storage and link management. Automated reminders and notifications are sent via Microsoft Outlook or Exchange. All integrations comply with County IT security policies, and no third-party or custom-built software components are required.


\subsection{Communications Interfaces}

All communications within the EPR Dashboard and Workflow System occur over secure, encrypted HTTPS/TLS 1.2 or higher protocols. User authentication and access are managed through the County’s Single Sign-On (SSO) system using OAuth 2.0 or SAML standards. Automated emails and notifications are sent through the County’s Microsoft Exchange network, ensuring data security and compliance with organizational IT policies.
...
This section describes the nonfunctional aspects of the EPR Dashboard and Workflow System, including performance, safety, security, software quality, and business requirements that guide how the system operates and is maintained.
...
\begin{enumerate}[label=\textbf{5.1.\arabic*}]
  \item The system shall process automated reminder notifications promptly after their scheduled trigger time.
  \item The system shall maintain fast and responsive dashboard performance under normal operating conditions, efficiently handling a high volume of employee performance records and reminder notifications.
...
\begin{enumerate}[label=\textbf{5.2.\arabic*}]
  ...
  \item The system shall ensure that failed automations or partial updates do not corrupt existing Smartsheet data or Box file links.
  \item The system shall maintain data consistency between integrated applications during recovery procedures.
\end{enumerate}

\subsection{Security Requirements}

This section defines the access control, authentication, and data protection measures necessary to safeguard sensitive employee information.
...
\begin{enumerate}[label=\textbf{5.3.\arabic*}]
  ...
  \item The system shall enforce role-based access control (RBAC), restricting each user’s access based on their assigned role.
  \item Sensitive EPR documents shall be stored only in Box, while Smartsheet retains metadata and secure links.
  \item The system shall comply with Santa Barbara County information security and data privacy policies.
  \item Power BI dashboards, if implemented, shall utilize Row-Level Security (RLS) to limit data visibility by role and division.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{5.4.\arabic*}]
  \item The system shall provide an intuitive and easy-to-use interface for HR staff and supervisors.
  ...
  \item The system shall be scalable, allowing new divisions, workflows, or automation processes to be added without major structural changes.
\end{enumerate}

\subsection{Business Rules}

This section outlines the operational rules and policies that govern the use of the EPR Dashboard and Workflow System.
...
\begin{enumerate}[label=\textbf{5.5.\arabic*}]
  ...
  \item Supervisors and HR staff may update review statuses but shall not delete EPR records.
  \item Only County-approved and current EPR templates shall be used for reviews.
...
6.1.1 Privacy and security are essential to ensure that the EPR Dashboard and Workflow System operates within the legal and ethical standards of Santa Barbara County.

\begin{itemize}
  \item 6.1.1.1 The system shall comply with all applicable County HR policies, data privacy regulations, and records retention requirements to protect sensitive employee information.
  \item 6.1.1.2 The system shall ensure that all employee performance data is securely collected, stored, and processed in accordance with County IT security standards.
  \item 6.1.1.3 Authorized personnel shall have access only to the data necessary for their role, following the principle of least privilege.
  \item 6.1.1.4 All personally identifiable information shall remain protected through access controls, encryption, and secure data transmission methods.
  \item 6.1.1.5 The system shall maintain audit logs of user activities, ensuring transparency and accountability in data handling.
\end{itemize}

\subsection{Ethical Considerations}

6.1.2 Ethical standards guide the responsible use of employee data and foster trust and fairness across the organization.

\begin{itemize}
  \item 6.1.2.1 The system shall handle all employee performance data ethically, ensuring that information is used solely for official HR purposes.
  \item 6.1.2.2 The system shall provide employees with clear communication regarding how their data is used, stored, and protected.
  \item 6.1.2.3 Development and maintenance of the system shall promote inclusivity, ensuring accessibility for users with varying abilities.
  \item 6.1.2.4 The system design shall include safeguards to prevent data misuse, unauthorized access, or exploitation of performance information.
\end{itemize}
...
Workflow and data flow diagrams (to be added).
