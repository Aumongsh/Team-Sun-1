...
8.0 & 04/28/2026 & James San & Added setup for all projects python scripts and AWS build and deploy steps. \\ \hline
\end{tabularx}
\end{center}
...
% =====================================
% Section 7 – Execution and Automation Model
% =====================================
\section{Execution and Automation Model}

This section defines the end-to-end automation architecture, including Smartsheet-native workflows and the external execution framework powered by AWS and Python.

\subsection{Workflow Automation Structure}

All workflows originate from Smartsheet and Box events, triggered via webhook integrations.

Core characteristics include:

\begin{itemize}
    \item Event-driven execution via Smartsheet and Box webhooks
    \item Status-based lifecycle management within tracker sheets
    \item Conditional logic enforced through control and validation fields
    \item Idempotent processing using flags to prevent duplicate execution
\end{itemize}

Each tracker (e.g., Separation, EPR, Personnel Matters) follows a consistent structural design, while incorporating workflow-specific fields tailored to each process.

Fields are generally organized into three categories:

\begin{itemize}
    \item \textbf{Core Fields:} Identify and track the record, including status, ownership, and key dates
    \item \textbf{Control Fields:} Support workflow execution through approvals, processing flags, and next-step indicators
    \item \textbf{Audit Fields:} Capture system activity, including timestamps, processing metrics, and execution logs
\end{itemize}

Specific field definitions vary by tracker to support the requirements of each workflow.

Smartsheet automation rules handle:

\begin{itemize}
    \item Record initialization from intake forms
    \item Status transitions and approvals
    \item Notifications and reminders
    \item Flagging records for downstream processing (e.g., archival, sync)
\end{itemize}

\subsection{Event-Driven Integration Architecture}

External processing is initiated via webhook events from:

\begin{itemize}
    \item Smartsheet (row creation/updates)
    \item Box (file/folder events)
\end{itemize}

These events are routed through AWS API Gateway, which directs requests to workflow-specific Lambda functions:

\begin{itemize}
    \item EPR Lambda
    \item Personnel Matters Lambda
    \item Separations Lambda
    \item Vacancies \& Recruitment Lambda
\end{itemize}

Each Lambda function invokes a corresponding Python script responsible for executing business logic.

\subsection{Python Execution Framework}

Python scripts perform operations not supported natively within Smartsheet, including:

\begin{itemize}
    \item Box folder creation and archival synchronization
    \item Cross-system data validation and enrichment
    \item Metadata extraction and transformation
    \item Dynamic email content generation
    \item Bulk and asynchronous record processing
\end{itemize}

\subsubsection{Execution Pattern}

Each Lambda-triggered Python process follows a consistent pattern:

\begin{enumerate}
    \item Retrieve event payload from API Gateway
    \item Authenticate using credentials from AWS Secrets Manager
    \item Identify and retrieve the relevant Smartsheet record(s)
    \item Validate required fields and processing conditions
    \item Execute workflow-specific actions (e.g., Box operations, updates)
    \item Write results back to Smartsheet (status updates, timestamps)
    \item Log execution outcomes for auditability
\end{enumerate}

Processing is designed to be idempotent, ensuring repeated events do not create duplicate artifacts.

\subsubsection{Security Controls}

\begin{itemize}
    \item Centralized credential management via AWS Secrets Manager
    \item OAuth 2.0 authentication for Smartsheet and Box APIs
    \item Least-privilege access for all services and scripts
    \item Structured logging within Lambda for traceability and monitoring
\end{itemize}

\subsection{Performance Characteristics}

\begin{itemize}
    \item Fully event-driven architecture with no manual triggers
    \item Serverless execution via AWS Lambda enables horizontal scaling
    \item Asynchronous processing prevents blocking within Smartsheet workflows
    \item Designed to operate within API rate limits for all integrated systems
\end{itemize}
...
The EPR Tracker serves as the operational data source, storing HR-managed evaluation records submitted through intake forms, including Employee ID, Review Period, Status, Reviewer, ratings, comments, approvals, outcomes, and processing time.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{PowerBI Dashboard HR.png}
    \caption{PowerBI HR Dashboard}
    \label{fig:PowerBI HR Dashboard}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{PowerBI Dashboard EPR.png}
    \caption{PowerBI EPR Dashboard}
    \label{fig:PowerBI EPR Dashboard}
\end{figure}
...
The Separation Tracker serves as the operational data source, storing HR-managed case records submitted through intake forms, including Employee ID, Separation Type, dates, Status, Reason, Department, Supervisor, approvals, outcomes, and processing time.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Separation PBI.png}
...
% =====================================
% Section 10 – Requirements Validation and Verification
% =====================================
...
The following section will demonstrate the step-by-step process for setting up all the project scripts and webhook automations. This process involves many different technologies, such as the Python programming language, Smartsheet SDK, Box.com SDK, and many AWS Services, and different credentials from Smartsheet, Box.com, and AWS.

\subsection{Prerequisites}
All of the following must be completed before moving on to the following steps:
\begin{itemize}
    \item Has administrative access to or a copy of the "SBPD - CSULA Spring 2026" Smartsheet workspace.
    \item Has administrative access or a copy of the "SBPD - CSULA" Box.com repository. If not, manually creating each folder and its subfolders will be required in step 11.2.
    \item Python version 3.11.4 installed locally.
    \item AWS SAM CLI installed locally.
\end{itemize}

\subsection{Obtaining API Keys}
    \subsubsection{Smartsheet}
    Obtaining a Smartsheet API key is simple. However, it is required to have Smartsheet's enterprise plan to generate an API key.
        \begin{enumerate}
            \item Navigate to \url{https://app.smartsheet.com/home}.
            \item After signing in, click on your user icon on the bottom left corner.
            \item Navigate to "Apps \& Integrations...", then "API Access", and then click the "Generate new access token" button.
            \item Create any name for the API key. Then save this string somewhere secure. This is your "Smartsheet API Key", we will be referencing it many times in this setup. Do not share this key with anyone!
        \end{enumerate}
    \subsubsection{Box.com}
    Obtaining a Box.com API key is slightly more difficult. However, Box.com allows you to generate an API key without any paid plan.
        \begin{enumerate}
            \item Navigate to \url{https://app.box.com/developers/console}.
            \item After signing in, click on the "New app" button on the top right corner.
            \item The "App Name" can be anything you want. However, select "Server Auth - JWT" for the App Type.
            \item Scroll down to "Developer Actions" and enable "Manage webhooks".
            \item Then click the "Generate Keypair" button to download the json file. Save this file somewhere secure on your desktop. This is your "Box.com API Credentials", we will be referencing it many times in this setup. Do not share this file with anyone!
            \item Ensure Box.com folder structure follows the following schema:
        \end{enumerate}
        \dirtree{%
            .1 SBPD - CSULA/.
            .2 Personnel Matters/.
            .2 Vacancies\&Recruitments/.
            .3 1\.\,ADD NEW DEN FILE HERE!!/.
            .3 2\.\,USED DEN FILES - SAFE TO DELETE/.
            .3 3\.\,INVALID DEN FILES - SAFE TO DELETE/.
            .2 EPR Tracker/.
            .3 EPR Attachments/.
            .2 Separations/.
            .3 IMPORTANT - ATTACHMENTS SENT TO ALL EXITS/.
            .3 email\_template.boxnote.
        }

\subsection{Setting up Developer Environment}
In this section, we will be pulling the source code from github and then ensuring all the proper dependencies are downloaded.
\\\textit{Note: Use Python 3.11.4 as other versions may not work.}
\begin{enumerate}
    \item First, pull the source code from GitHub.
        \begin{verbatim}git pull https://github.com/PickolZi/CS4961-SBPD\end{verbatim}
    \item Then, create and run a virtual environment to download the python dependencies.
        \begin{verbatim}python -m venv venv\end{verbatim}
        \begin{verbatim}source venv/Scripts/activate\end{verbatim}
    \item Last, download all the python dependencies to the virtual environment.
        \begin{verbatim}pip install -r requirements.txt\end{verbatim}
\end{enumerate}

\subsection{Setting up Project Credentials}
After the previous steps, all the internal CLI tools and scripts can run, however, they will fail due to credentials and configuration errors. In this section, we will add the credentials to the .env file to get half of the internal CLI tools working.
\begin{enumerate}
    \item Rename the '.env.template' file to '.env'.
    \item Set 'SMARTSHEET\_ACCESS\_TOKEN' to the Smartsheet API Key you saved from step 11.2.1
    \item Using the JSON file you downloaded from Box.com during step 11.2.2, set the following values from the JSON file to the .env values.
        \begin{itemize}
            \item BOX\_CLIENT\_ID
            \item BOX\_CLIENT\_SECRET
            \item BOX\_JWT\_KEY\_ID
            \item BOX\_PRIVATE\_KEY
            \item BOX\_PRIVATE\_KEY\_PASSPHRASE
            \item BOX\_ENTERPRISE\_ID
        \end{itemize}
    \item Specifically for the Separations project, you can generate the Gmail credentials using the following link: \url{https://myaccount.google.com/apppasswords}. After generating the credentials, set both the 'GMAIL\_SENDER\_ADDRESS' and 'GMAIL\_APP\_PASSWORD'.
\end{enumerate}

\subsection{Setting up Project Configuration}
The project configuration file \path{layers/shared/python/shared_config/config.py} is the centralized location for all project scripts to find which column to pull data from. And in order to figure out what the column ids are, we need to use an internal CLI tool we developed suited just for this purpose.
\subsubsection{Filling in WebhookCLI Configuration class}
    \begin{enumerate}
        \item Activate the smartsheet\_ids\_cli.py script and list all table IDs.
            \begin{verbatim}source venv/Scripts/activate\end{verbatim}
            \begin{verbatim}python smartsheet_ids_cli.py\end{verbatim}
            \begin{verbatim}> List Table IDs\end{verbatim}        
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.1_smartsheet_ids_cli.png}
                \caption{smartsheet\_ids\_cli.py Listing Table IDs}
                \label{fig:placeholder}
            \end{figure}
        \item Navigate to Box.com's folder titled "SBPD - CSULA / Vacancies\&Recruitments / 1. ADD NEW DEN FILE HERE!!" And copy the folder's ID from the address bar.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.1_box_upload_folder_id.png}
                \caption{Vacancies \& Recruitment Project - DEN Upload Folder ID}
                \label{fig:placeholder}
            \end{figure}
        \item Overall, this is what the WebhookCLI class should look like.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.1_webhookcli_class_config.png}
                \caption{Webhook CLI Class Config}
                \label{fig:placeholder}
            \end{figure}
    \end{enumerate}
\subsubsection{Filling in EPRTracker Configuration class}
    \begin{enumerate}
        \item Close and re-open the smartsheet\_ids\_cli.py script and then List EPR Tracker Column IDs.
            \begin{verbatim}python smartsheet_ids_cli.py\end{verbatim}
            \begin{verbatim}> List EPR Tracker Column IDs\end{verbatim}        
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.2_smartsheet_epr_tracker_column_ids.png}
                \caption{smartsheet\_ids\_cli.py Listing EPRTracker column IDs}
                \label{fig:placeholder}
            \end{figure}
        \item Overall, this is what the EPRTracker class should look like.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.2_EPRTracker_class_config.png}
                \caption{EPRTracker Class Config}
                \label{fig:placeholder}
            \end{figure}
    \end{enumerate}
\subsubsection{Filling in Separations Configuration class}
    \begin{enumerate}
        \item List Separations Column IDs.
            \begin{verbatim}> List Separations Tracker Column IDs\end{verbatim}        
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.3_smartsheet_separations_column_ids.png}
                \caption{smartsheet\_ids\_cli.py Listing Separations column IDs}
                \label{fig:placeholder}
            \end{figure}
        \item Navigate to Box.com's Separations folder and attach both the email template boxnote file and the important attachments folder.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.3_box_email_template_id.png}
                \caption{Separations Project - Box Email Template Boxnote File ID}
                \label{fig:placeholder}
            \end{figure}
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.3_box_important_attachments_folder_id.png}
                \caption{Separations Project - Box Important Attachments Folder ID}
                \label{fig:placeholder}
            \end{figure}
        \item Overall, this is what the Separations class should look like.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.3_Separations_class_config.png}
                \caption{Separations Class Config}
                \label{fig:placeholder}
            \end{figure}
    \end{enumerate}
\subsubsection{Filling in Vacancies\&Recruitment Configuration class}
    \begin{enumerate}
        \item List Vacancies\&Recruitment Column IDs.
            \begin{verbatim}> List Vacancies & Recruitment Tracker Column IDs\end{verbatim}        
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.4_smartsheet_vacancies_column_ids.png}
                \caption{smartsheet\_ids\_cli.py Listing Vacancies \& Recruitment column IDs}
                \label{fig:placeholder}
            \end{figure}
        \item Navigate to Box.com's Vacancies\&Recruitments folder and attach all 3 folder ids.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.4_box_upload_folder_id.png}
                \caption{Vacancies\&Recruitments Project - Box Upload Folder ID}
                \label{fig:placeholder}
            \end{figure}
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.4_box_used_folder_id.png}
                \caption{Vacancies\&Recruitments Project - Box Used Folder ID}
                \label{fig:placeholder}
            \end{figure}
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.4_box_invalid_folder_id.png}
                \caption{Vacancies\&Recruitments Project - Box Invalid Folder ID}
                \label{fig:placeholder}
            \end{figure}
        \item Overall, this is what the Vacancies\&Recruitments  class should look like.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.4_Vacancies_class_config.png}
                \caption{Vacancies\&Recruitments Class Config}
                \label{fig:placeholder}
            \end{figure}
    \end{enumerate}    
\subsubsection{Filling in Personnel Matters Configuration class}
    \begin{enumerate}
        \item List Personnel Matters Column IDs.
            \begin{verbatim}> List Personnel Matters Column IDs\end{verbatim}        
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.5_smartsheet_personnel_matters_column_ids.png}
                \caption{smartsheet\_ids\_cli.py Listing Personnel Matters column IDs}
                \label{fig:placeholder}
            \end{figure}
        \item Navigate to Box.com's Personnel Matters folder and attach that folder.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.5_box_personnel_matters_folder_id.png}
                \caption{Personnel Matters Project - Box Personnel Matters Folder ID}
                \label{fig:placeholder}
            \end{figure}
        \item Overall, this is what the Personnel Matters class should look like.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\linewidth]{11.5.5_personnel_matters_class_config.png}
                \caption{Personnel Matters Class Config}
                \label{fig:placeholder}
            \end{figure}
    \end{enumerate}

\subsection{Deploying AWS Stacks using AWS SAM}
Deploying the AWS Stacks allows the python scripts to be hosted on AWS Lambda, which are Amazon's serverless compute instances. Doing this allows Smartsheet and Box.com to send requests to our AWS lambdas and let them know when a new event has occurred, causing the scripts to execute without manually intervention. 
\begin{enumerate}
    \item Repackage the layers into a zip file. Anytime any changes are made to shared\_config/ folder, the parent python folder needs to be re-zipped. Then the following commands must be run.
        \begin{verbatim}sam build\end{verbatim}
        \begin{verbatim}sam deploy\end{verbatim}
    \item After saying "yes" to their stack changes, the cloud formation stacks should create or update the stacks in AWS. Which will either create or update the API Gateway, IAM roles, AWS Lambdas, and Secrets Manager services.
    \item Next, in order to give the lambdas credentials to run the script, you need to upload the .env credentials into AWS SecretsManager.
        \begin{figure}[H]
            \centering
            \includegraphics[width=1\linewidth]{11.6_secrets_manager_1.png}
            \caption{AWS Secrets Manager}
            \label{fig:placeholder}
        \end{figure}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.5\linewidth]{11.6_secrets_manager_2.png}
            \caption{AWS Secrets Manager}
            \label{fig:placeholder}
        \end{figure}
    \item Before we turn on the webhooks that allow Smartsheet and Box.com to send requests to our lambdas, we first need to add the endpoints of our lambdas to our .env file. And we can find the endpoints by navigating to AWS API Gateway.
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{11.6_api_gateway.png}
        \caption{AWS API Gateway endpoints}
        \label{fig:placeholder}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{11.6_env_api_gateway_endpoints.png}
        \caption{.env AWS API Gateway endpoints}
        \label{fig:placeholder}
    \end{figure}
    \item Finally, we can turn on all the webhook endpoints by running the following commands.
        \begin{verbatim}python webhook_cli.py\end{verbatim}
        \begin{verbatim}> Create Webhook\end{verbatim}
        \begin{verbatim}> SMARTSHEET) EPR Tracker\end{verbatim}
        \begin{verbatim}> SMARTSHEET) Personnel Matters\end{verbatim}
        \begin{verbatim}> SMARTSHEET) Separations Tracker\end{verbatim}
        \begin{verbatim}> BOX) Vacancies & Recruitment Tracker\end{verbatim}
\end{enumerate}
