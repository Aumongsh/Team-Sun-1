...
5.0 & 03/23/2026 & Momoka Aung &  Added Section 9  Database Design. \\ \hline
\end{tabularx}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Microsoft Outlook/Exchange}: Sends automated notifications and reminders.
\end{itemize}
...
Implements review creation, supervisor assignment, sequential approval routing, digital acknowledgment tracking, completion state management, and archival initiation.

\paragraph{Personnel Matters Subsystem}
Implements structured administrative record tracking, document association, metadata extraction (if enabled), approval routing, and archival initiation.

\paragraph{Vacancies and Recruitment Subsystem}
Implements vacancy creation, status lifecycle management, milestone tracking, approval logging (if configured), and analytics exposure.
...
Smartsheet functions as the operational workflow engine and system of record for structured HR workflow data. It manages workflow state, routing logic, and automation rules through controlled \texttt{Status} fields.

Only structured metadata is stored within Smartsheet. Finalized document artifacts are stored externally, with references (e.g., \texttt{Box\_URL}) maintained in workflow records.
...
Analytics data is refreshed on a scheduled basis rather than in real time. Nightly Power BI refresh cycles provide sufficient reporting accuracy while reducing API usage and operational overhead.
...
External systems are accessed through APIs and platform connectors to isolate workflow logic from vendor-specific implementations. Within Power BI, reusable DAX measures encapsulate business logic to simplify report maintenance and ensure consistency across dashboards.
...
\begin{itemize}[nosep]
    \item Python automation for archival validation
    \item Box document upload and storage
    \item Power BI dataset refresh for reporting
\end{itemize}
...
\paragraph{Integration Points}
\begin{itemize}[nosep]
    \item Optional document metadata extraction via Python
...
\paragraph{Integration Points}
\begin{itemize}[nosep]
    \item Optional document extraction via Box webhook and Python
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Personnel Matters Employee Intake Form.png}
    \caption{Personnel Matters Employee Intake Form}
    \label{fig:Personnel Matters Employee Intake Form}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Personnel Matters HR Intake Form.png}
    \caption{Personnel Matters HR Intake Form}
    \label{fig:Personnel Matters HR Intake Form}
\end{figure}

\begin{itemize}[nosep]
    \item \textbf{Smartsheet} — Operational workflow interface
    \item \textbf{Box} — Document storage interface
    \item \textbf{Power BI} — Reporting and analytics dashboards
\end{itemize}

All UI configuration is performed within these platforms.

\subsection{Primary User Interfaces}
