...
0.1 & 08 Sep 2025 & Momoka Aung, Chiemela Eziechile - Nwoke & Initial draft created. \\ \hline
...
0.4 & 3 Dec 2025 & Momoka Aung & Added workflow diagram. \\ \hline
0.5 & 30 Jan 2026 & Momoka Aung & Added information of Recruitments, Vacancies, and Separation projects.  \\ \hline
\end{tabular}
\end{center}
...
This Software Requirements Specification (SRS) describes the key features and expectations for the Human Resources Workflow Management System, which includes both the Employee Performance Review (EPR) Dashboard, the Personnel Matters, Recruitments, Vacancies, and Separation Tracker projects. The system is being developed for the Santa Barbara County Public Defender’s Office to improve how HR processes are tracked, managed, and reported.
...
This Software Requirements Specification (SRS) defines the functional and nonfunctional requirements for the Human Resources Workflow Management System, which includes the Employee Performance Review (EPR) Dashboard and the Personnel Matters, Recruitments, Vacancies, and Separation Tracker projects. 
...
The Human Resources Workflow Management System is designed to modernize and centralize two key HR functions within the Santa Barbara County Public Defender’s Office: the Employee Performance Review (EPR) Dashboard and the Personnel Matters, Recruitments, Vacancies, and Separation Tracker projects. Together, these tools streamline HR processes, reduce manual workload, and improve transparency and accountability across all divisions.
...
The \textbf{Vacancies & Recruitments Dashboard} centralizes the tracking of open positions and hiring activity, providing leadership with real-time visibility and HR with a standardized management tool. It replaces manual downloads from the Den application with a Smartsheet-based tracker that captures key recruitment data such as job class, status, funding, vacancy dates, and time-to-hire. This data feeds a Power BI dashboard that presents active and historical vacancy trends, recruitment statuses, new hires, interview activity, and internal versus external recruitments to support informed workforce planning.

The \textbf{Separations Dashboard} standardizes the capture and reporting of employee exits through a centralized Smartsheet tracker with automated field dependencies to improve data accuracy. It supports consistent separation notifications and feeds a Power BI dashboard that provides real-time insights into exits by job class and reason, with flexible time-based filtering. This component replaces informal, email-based processes with a consistent, auditable workflow that improves transparency, reporting, and leadership awareness.
...
This section provides a high-level understanding of the Human Resources Workflow Management System, including the Employee Performance Review (EPR) Dashboard, the Personnel Matters, Recruitments, Vacancies, and Separation Tracker projects outlining how each component is used, the environment in which they operate, and the key factors influencing their development and deployment.
...
Vacancies, recruitments, and employee separations present comparable operational challenges. At present, vacancy and recruitment activity is not tracked in a centralized system, requiring HR to manually download reports from the county-provided Den application and manage updates outside of a structured workflow. Similarly, employee separations are communicated informally between supervisors and HR, resulting in inconsistent documentation and limited reporting capabilities.

The proposed Vacancies \& Recruitments and Separations dashboards extend the same Smartsheet and Power BI architecture to workforce lifecycle management. Smartsheet grids serve as the system of record for recruitment and separation data, capturing standardized fields such as position details, job class, status, dates, and reasons for exit, with automated field dependencies to reduce manual entry. Power BI dashboards provide leadership with real-time visibility into vacancies, hiring activity, and separation trends through interactive charts, counts, and time-based filters. Together, these systems replace fragmented, manual processes with a consistent, auditable, and data-driven approach to workforce planning and HR reporting.
...
\item \textbf{2.3.5 Personnel Matters Case Tracking}
  ...
  \end{itemize}
    ...
    \item \textbf{2.3.5 Vacancies and Recruitments Tracking}
  \begin{itemize}
  ...
  \item \textbf{2.3.6 Employee Separations Tracking and Reporting}
  \begin{itemize}
...
2.5.6 Access will be restricted to authorized employees, supervisors, HR staff, and administrators within the Santa Barbara County Public Defender’s Office, with permissions defined separately for EPR, Personnel Matters, Vacancies and Recruitments, Employee Separations Tracking workflows.
...
The EPR Dashboard, Personnel Matters, Vacancies and Recruitments, Employee Separations Tracker will include a comprehensive set of user documentation to support onboarding, daily operation, and long-term maintenance. This Software Requirements Specification (SRS) will serve as a reference guide for understanding the system’s structure, functionality, and configuration procedures.  
...
Phase 1: Implementation of the EPR Workflow and Dashboard, including centralized tracking, automated reminders and escalations, Power BI reporting, and secure document management through Box.

Phase 2: Deployment of the Personnel Matters Tracker, introducing standardized intake forms, structured case tracking, automated status management, and centralized documentation to replace informal, email-based processes.

Phase 3: Implementation of the Vacancies \& Recruitments Tracker and dashboard, providing centralized tracking of open positions and hiring activity, automated data intake from the Den application, and real-time Power BI analytics to support workforce planning.

Phase 4: Deployment of the Employee Separations Tracker and dashboard, including standardized separation data capture, automated notification emails, and leadership reporting on exit trends by job class and reason.

% ===================== 3. External Interface Requirements =====================
\section{External Interface Requirements}

This section outlines how the EPR Dashboard, Personnel Matters, Vacancies and Recruitments, Employee Separations Tracker will interact with external interfaces, including user interfaces, hardware, software, and communication channels. These requirements describe how both components operate within the Santa Barbara County Public Defender’s Office’s IT environment and ensure seamless integration with existing enterprise tools.
...
All communications within the EPR Dashboard, Personnel Matters, Vacancies and Recruitments, Employee Separations Tracker occur over secure, encrypted HTTPS/TLS 1.2 or higher protocols. User authentication and access are managed through the County’s Single Sign-On (SSO) system using OAuth 2.0 or SAML standards. Automated emails and notifications are sent through the County’s Microsoft Exchange network, ensuring data security and compliance with organizational IT policies.
...
\subsubsection{Vacancies and Recruitments Workflow Management} % 4.1.6
\begin{enumerate}[label=\thesubsubsection.\arabic*, left=0pt]
  \item The system shall maintain a centralized Smartsheet Vacancies and Recruitments Tracker to record and monitor all active vacancies and recruitment activities. The tracker shall include the following minimum fields:
    \begin{itemize}
      \item Position ID
      \item Job Class
      \item Office Location
      \item Recruitment Status – Indicates the current stage of recruitment (e.g., Interview Phase, Offer Letter Phase).
      \item Funding Status – Identifies whether the position is Funded or Unfunded.
      \item Vacancy Start Date – Date the vacancy was opened.
      \item Date Filled – Date the position was filled.
      \item Time to Hire – Calculated number of calendar days between Vacancy Start Date and Date Filled.
    \end{itemize}

  \item The system shall support initial data population through uploads of vacancy and recruitment reports exported from the county-provided Den application, with HR completing or updating remaining fields manually.

  \item The system shall automatically calculate Time to Hire based on Vacancy Start Date and Date Filled and update the value when either date changes.

  \item The system shall allow HR to update recruitment status as a vacancy progresses through defined stages, ensuring real-time visibility into hiring activity.

  \item The system shall support classification of recruitments as Internal or External and capture this designation for reporting and analytics purposes.

  \item The system shall restrict access to vacancy and recruitment records based on role:
    \begin{itemize}
      \item HR/Admin – Full access to create, edit, and report on records.
      \item Leadership – Read-only access to dashboards and summary views.
    \end{itemize}

  \item The system shall provide structured data outputs to Power BI to support dashboards summarizing active vacancies, recruitment status distribution, new hires, interview activity, office location breakdowns, and historical hiring trends.
\end{enumerate}

\subsubsection{Employee Separations Workflow Management} % 4.1.7
\begin{enumerate}[label=\thesubsubsection.\arabic*, left=0pt]
  \item The system shall maintain a centralized Smartsheet Separation Tracker to record and monitor all employee separations. The tracker shall include the following minimum fields:
    \begin{itemize}
      \item Staff Name
      \item Staff Email
      \item Position ID
      \item Job Class
      \item Reason for Exit
      \item Date Submitted – Date the separation was recorded by HR.
      \item Last Day – Employee’s final working day.
    \end{itemize}

  \item The system shall allow HR staff to create and update separation records using a Smartsheet form or direct grid entry to ensure standardized and consistent data capture.

  \item The system shall enforce completion of required fields (Position ID, Reason for Exit, Date Submitted, and Last Day) before a separation record can be finalized or included in reporting.

  \item The system shall support automated field dependencies to reduce manual data entry, including auto-populating Staff Name and Job Class based on Position ID or auto-populating Position ID and Job Class based on Staff Name, when reference data is available.

  \item The system shall generate a standardized separation notification email using a predefined template populated with dynamic fields from the Separation Tracker.

  \item The system shall log each separation notification event with timestamps, recipients, and delivery status for audit and compliance purposes.

  \item The system shall restrict access to separation records based on role:
    \begin{itemize}
      \item HR/Admin – Full access to create, edit, and report on records.
      \item Leadership – Read-only access to dashboards and summary views.
    \end{itemize}

  \item Separation records shall be retained in accordance with County data retention and privacy policies, and any supporting documentation shall be stored securely in Box and referenced by hyperlink when applicable.
\end{enumerate}
...
This section describes the nonfunctional aspects of the Human Resources Workflow Management System, including the Employee Performance Review (EPR) Dashboard and the Personnel Matters, Vacancies and Recruitments, Employee Separations Tracker. These requirements cover performance, safety, security, software quality, and business rules that guide how the system operates and is maintained.
...
This section outlines the operational rules and policies that govern the use of the EPR Dashboard, Personnel Matters Tracker, Vacancies and Recruitments Tracker, and Employee Separations Tracker.
...
\begin{enumerate}[label=\textbf{5.5.\arabic*}]
  ...
  \item Only HR administrators shall have the ability to configure templates, reminders, escalation rules, and automated notifications across all HR tracking systems.
  \item Supervisors and HR staff may update review or recruitment statuses but shall not delete EPR, Personnel Matters, Vacancy, Recruitment, or Separation records.
  \item Only County-approved and current templates shall be used for performance reviews, personnel case documentation, recruitment communications, and separation notifications.
  \item Escalation recipients, reporting hierarchies, and approval roles shall be maintained within the Supervisor Master sheet and reviewed quarterly by HR.
  \item All vacancy, recruitment, and separation data shall be entered and maintained by HR to ensure consistency, accuracy, and compliance with County workforce reporting standards.
  \item Separation records and notifications shall be completed in a timely manner following an employee’s exit and retained in accordance with County data retention and privacy policies.
\end{enumerate}
...
\textbf{Vacancies and Recruitments Tracker:} A Smartsheet-based tracking tool used by HR to manage and monitor open positions and recruitment activity.

\textbf{Separation Tracker:} A Smartsheet-based tracking tool used by HR to record and manage employee separations.
